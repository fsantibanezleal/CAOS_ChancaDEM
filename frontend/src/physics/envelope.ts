...
export function envelopeText(code: EnvelopeCode, es: boolean): string {
  const T: Record<EnvelopeCode, { en: string; es: string }> = {
    ...
    css_outside_calibrated: {
      ...
      es: `CSS fuera del soporte calibrado HP500 [${HP500_ENVELOPE.cssMm[0]}, ${HP500_ENVELOPE.cssMm[1]}] mm: el ajuste (Tabla 5) está extrapolando.`,
    },
    f80_outside_calibrated: {
      ...
      es: `f80 fuera del soporte calibrado HP500 [${HP500_ENVELOPE.f80Mm[0]}, ${HP500_ENVELOPE.f80Mm[1]}] mm: el ajuste (Tabla 5) está extrapolando.`,
    },
  };
