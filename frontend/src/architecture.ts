  ...
  tabs: [
    {
      ...
      body_en:
        'ChancaDEM is a comminution product: the controls select a crusher (cone / jaw / gyratory) and set the ' +
        'closed-side setting, throw, eccentric speed and feed PSD, and the product gradation (P80/P50), the throughput ' +
        '(tph) and the Bond power (kW) form, live, in a 3-D chamber view. It answers "how do the chamber settings shape ' +
        'the product size, the capacity and the power?".\n\n' +
        ...
        'recommended CSS" is solved by bisection on the monotone surrogate. Contract 1 accepts a user-supplied operating point.',
      body_es:
        'ChancaDEM es un producto de conminución: los controles seleccionan un chancador (cono / mandíbula / giratorio) ' +
        'y fijan el closed-side-setting, el throw, la velocidad del excéntrico y la PSD de alimentación, y la ' +
        'gradación del producto (P80/P50), el throughput (tph) y la potencia de Bond (kW) se forman en vivo en una ' +
        'cámara 3-D. Responde "¿cómo moldean los ajustes de cámara el tamaño del producto, la capacidad y la potencia?".\n\n' +
        ...
        'CSS recomendado" se resuelve por bisección sobre el surrogate monótono. El Contrato 1 acepta un punto de operación proporcionado por el usuario.',
    },
    {
      id: 'lanes',
      ...
      body_en:
        ...
        'server. Offline / compute (the local machine, isolated .venv): the Python pipeline precomputes the canonical ' +
        'case artifacts and the heavy lane (--retrain, .venv-precompute, torch) trains the surrogate + the autoencoder ' +
        'and exports them to ONNX. Replay: the small, committed artifacts in data/derived are overlaid into the SPA by ' +
      ...
      body_es:
        ...
        '(frontend/src/physics/) vuelve a ejecutarse con cada control y onnxruntime-web ejecuta surrogate.onnx + psd-ae.onnx, sin ' +
        'servidor. Offline / cómputo (la máquina local, .venv aislado): el pipeline Python precalcula los artefactos ' +
        'canónicos por caso y el carril pesado (--retrain, .venv-precompute, torch) entrena el surrogate + el ' +
        'autoencoder y los exporta a ONNX. Replay: los artefactos pequeños y versionados en data/derived se superponen ' +
        'al SPA con copy-data.mjs y se cargan en vivo; el espejo tipado (contract.types.ts) rompe el build si la web y el pipeline divergen.',
    },
    {
      id: 'web-flow',
      en: 'Web-app flow',
      ...
      body_en:
        'The App page recomputes live: inputs (the case selector or a user-supplied operating point, plus the machine, CSS, ' +
      ...
      body_es:
        'La página App recalcula en vivo: las entradas (el selector de casos o un punto de operación proporcionado por el usuario, más los ' +
    ...
    },
    {
      ...
      body_es:
        ...
        'P80→CSS por bisección, y un autoencoder denoising marca puntos de operación fuera de la envolvente de entrenamiento. ' +
        'Ambos se ejecutan en el cliente como ONNX, reportados junto a la física, nunca como caja negra.',
    },
    {
      id: 'design',
      ...
      body_en:
        ...
        'the machine, CSS, throw, speed and feed descriptors, with range/NaN guards, so the app accepts user-supplied data, not ' +
      ...
      body_es:
        ...
        'la app acepte datos del usuario, no sólo los casos incluidos. El Contrato 2 (artefacto) define la salida que lee la web ' +
        '(puntos de operación por caso, la precisión surrogate-vs-física, el índice de modelos), espejada exactamente por ' +
        'contract.types.ts. Entre ambos, el pipeline por etapas y determinista ejecuta el lane gate (numpy-light por ' +
